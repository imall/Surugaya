...
{
    private readonly ILogger<LetaoCartService> _logger;
    private readonly LetaoSettings _settings;
    private readonly LetaoAuthService _authService;

    public LetaoCartService(
    ...
    public async Task<LetaoAddToCartResponse> AddToCartAsync(LetaoAddToCartRequest request)
    {
        for (int attempt = 1; attempt <= _settings.MaxRetries; attempt++)
        {
            try
            {
                _logger.LogInformation("🔄 嘗試第 {Attempt} 次加入購物車...", attempt);

                // 取得有效的 Cookie（自動處理登入和快取）
                var cookieContainer = await _authService.GetValidCookieAsync();
                ...
                {
                    _logger.LogWarning("⚠️ 第 {Attempt} 次嘗試失敗：Session 已失效", attempt);
                ...
                if (result.Success)
                {
                    _logger.LogInformation("✅ 成功加入購物車！");
                    return result;
                }

                // 其他失敗原因
                _logger.LogWarning("❌ 加入購物車失敗：{Message}", result.Message);
                return result;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "❌ 第 {Attempt} 次嘗試發生錯誤", attempt);
        ...
        CookieContainer cookieContainer)
    {
        ...
        _logger.LogDebug("HTTP Status Code: {StatusCode}", response.StatusCode);
        _logger.LogDebug("API Response: {Response}", decoded);
    ...
    private bool IsSessionExpired(LetaoAddToCartResponse response)
    {
        ...
        {
            _logger.LogWarning("❌ HTTP 狀態碼顯示未授權");
            return true;
        }
        ...
        {
            _logger.LogWarning("❌ 回應內容包含登入失效關鍵字");
            return true;
        }
        ...
        try
        {
            ...
            if (json.RootElement.TryGetProperty("code", out var codeElement))
            {
                ...
                if (code != "200" && code != "0")
                {
                    _logger.LogWarning("❌ API 回傳失敗代碼: {Code}", code);

                    // 嘗試取得錯誤訊息
                    if (json.RootElement.TryGetProperty("message", out var msgElement))
                    {
                        _logger.LogWarning("   錯誤訊息: {Message}", msgElement.GetString());
                    }
        ...
        catch
        {
            ...
            {
                _logger.LogWarning("❌ 回應是 HTML 頁面（可能被重導向到登入頁）");
                return true;
            }
        }
