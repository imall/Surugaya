...
{
    private readonly ILogger<LetaoAuthService> _logger;
    ...
    public async Task<CookieContainer> GetValidCookieAsync()
    {
        lock (_lockObject)
        {
            ...
            {
                _logger.LogInformation("✅ 使用快取的 Cookie");
                return _cachedCookies;
            }
        }

        // Cookie 不存在或已過期，重新登入
        _logger.LogInformation("🔑 Cookie 不存在或已過期，重新登入...");
        var cookieContainer = await LoginAndGetCookiesAsync();

        lock (_lockObject)
        {
            _cachedCookies = cookieContainer;
            _cookiesExpireTime = DateTime.Now.AddHours(_settings.CookieCacheHours);
        }
    ...
    public void ClearCache()
    {
        lock (_lockObject)
        {
            ...
            _logger.LogInformation("🗑️ 已清除 Cookie 快取");
        }
    }
    ...
    private async Task<CookieContainer> LoginAndGetCookiesAsync()
    {
        ...
        {
            _logger.LogError("❌ 登入失敗: {Result}", result);
        ...
        {
            _logger.LogError("❌ 登入回應缺少 GUID 或過期時間");
            throw new Exception("登入回應格式錯誤");
        }

        _logger.LogInformation("✅ 登入成功！GUID: {Guid}", guid);
