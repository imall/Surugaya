...
public class SurugayaPurchaseController : ControllerBase
{
  ...
  public async Task<IActionResult> AddPurchase([FromBody] AddPurchaseRequest request)
  {
    try
    {
      ...
      if (result.Success)
      {
        _logger.LogInformation("✅ 購買紀錄新增成功");
        return Ok(result);
      }
      else
      {
        _logger.LogWarning("❌ 購買紀錄新增失敗: {Message}", result.Message);
        return BadRequest(result);
      }
    }
    catch (Exception ex)
    {
      _logger.LogError(ex, "新增購買紀錄時發生未預期的錯誤");
      return StatusCode(500, new
      {
        success = false,
        message = $"新增購買紀錄時發生未預期的錯誤：{ex.Message}"
      });
    }
  }
  ...
  public async Task<IActionResult> GetAllPurchases()
  {
    ...
    {
      _logger.LogError(ex, "取得購買紀錄時發生未預期的錯誤");
      return StatusCode(500, new
      {
        success = false,
        message = $"取得購買紀錄時發生未預期的錯誤：{ex.Message}"
      });
    }
  }
